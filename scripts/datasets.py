...
from robustness.tools import openimgs_helpers
...
class WILDSDataSet(object):
    ...
    def __init__(self, ds_name, data_path, **kwargs):
        ...
        required_args = ['num_classes', 'mean', 'std', 
                         'transform_train', 'transform_test']
        optional_args = ['custom_class', 'label_mapping', 'domain_mapping', 'custom_class_args']
...
class WILDSCustomImageNet(WILDSDataSet):
    '''
    ...
    '''
    def __init__(self, data_path, custom_grouping, domain_mapping, **kwargs):
        """
        """
        ...
        ds_kwargs = self.override_args(ds_kwargs, kwargs)
        super(WILDSCustomImageNet, self).__init__(ds_name,
                data_path, **ds_kwargs)
...
class CustomCIFAR100(torch.utils.data.Dataset):
    def __init__(self, data, targets, metadata, transform):
        self.data = data
    ...
    def __getitem__(self, idx):
        return self.transform(im.fromarray(self.data[idx])), self.targets[idx], self.metadata[idx]
